...
const Dashboard = () => {
  ...
  const [addresses, setAddresses] = useState([
    ...
    },
    {
      ...
      phone: '+91 9988776655',
      address_line1: 'Tech Tower, 5th Floor',
      ...
      latitude: '28.6304',
      longitude: '77.3819',
      delivery_instructions: 'Deliver between 9 AM and 6 PM',
      address_type: 'Office',
      is_default: false,
  ...
  const handleUpiSubmit = (e) => {
    ...
    if (!newUpiId.includes('@')) {
      openDialog("Invalid UPI ID", "Please enter a valid UPI ID (e.g. username@bank)", "⚠️");
      return;
    }
        ...
        <main className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-grow">
            ...
            <div className="animate-fade-in space-y-6 text-left">
              ...
              {wishlist.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                  {wishlist.map(w => {
                    const product = initialProducts.find(p => p.id === w.product_id);
                  ...
                  <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-2">
                    ...
                    <div className="flex md:flex-col items-center gap-3 md:gap-2 flex-1 relative">
                      ...
                      <div className="text-left md:text-center">
                        ...
                        <span className="text-[9px] text-slate-450 block font-semibold">Pending</span>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 gap-6 animate-fade-in text-left">
              ...
              {activeOrder ? (
                ...
                <div className="flex flex-col items-center justify-center gap-3 bg-slate-50 border border-dashed border-slate-200 rounded-3xl p-8">
                  ...
                  <button
                    onClick={() => setCurrentPage('store')}
                    className="mt-1 px-4 py-2 bg-brand-green text-white text-xs font-black rounded-xl hover:bg-emerald-700 transition-colors"
                  >
                    Shop Now →
                  </button>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {orders.map((ord, idx) => {
                        ...
                        const itemsText = items.map(oi => {
                          const p = initialProducts.find(prod => prod.id === oi.product_id);
                          return p ? `${p.name} (x${oi.quantity})` : 'Product';
                        }).join(', ');
                        return (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="py-4 px-2 font-black text-slate-800">{ord.order_number}</td>
                          ...
                          <td className="py-4 px-2 text-center">
                            <span className={`px-2.5 py-0.5 text-[9px] border font-black tracking-wide rounded-full ${
          ...
          {activeSidebarTab === 'reviews' && (
            <div className="animate-fade-in space-y-6 text-left">
              ...
              <div className="space-y-4">
                {reviews.map(rev => {
                  const product = initialProducts.find(p => p.id === rev.product_id);
                  if (!product) return null;
                  return (
